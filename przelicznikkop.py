...
class RegattaSetupWindow(tk.Toplevel):
    def __init__(self, master, race_results_app):
        super().__init__(master)
        self.title("Setup Regatta")

        self.race_results_app = race_results_app  # Dodajemy referencję do RaceResultsApp
        self.regatta_name = tk.StringVar()
        self.stage_number = tk.StringVar()

        ttk.Label(self, text="Nazwa regat:").grid(row=0, column=0, padx=10, pady=10, sticky="w")
        ttk.Entry(self, textvariable=self.regatta_name).grid(row=0, column=1, padx=10, pady=10)

        ttk.Label(self, text="Numer etapu:").grid(row=1, column=0, padx=10, pady=10, sticky="w")
        ttk.Entry(self, textvariable=self.stage_number).grid(row=1, column=1, padx=10, pady=10)

        ttk.Button(self, text="Ustaw", command=self.set_regatta_and_close).grid(row=2, column=0, columnspan=2, pady=10)

    def set_regatta_and_close(self):
        if self.regatta_name.get():
            self.race_results_app.setup_main_window(self.regatta_name.get(), self.stage_number.get())
            self.destroy()
        else:
...
class RaceResultsApp:
    def __init__(self, master):
     ...
     #   self.entry_regatta.grid(row=0, column=1, padx=10, pady=10)
       ...
       # self.set_regatta_button = ttk.Button(self.tab_regatta, text="Ustaw nazwę regat", command=self.regatta_name)
 ...
 # Przekazujemy referencję do samego siebie

    def setup_main_window(self, regatta_name, stage_number):
        if not regatta_name or not stage_number:
            return
        ...
        self.label_regatta = ttk.Label(self.tab_regatta, text=f"Nazwa regat: {regatta_name}")
        self.label_regatta.grid(row=0, column=0, padx=10, pady=10, sticky="w")

        self.label_stage = ttk.Label(self.tab_regatta, text=f"Numer etapu: {stage_number}")
        self.label_stage.grid(row=1, column=0, padx=10, pady=10, sticky="w")

    def set_regatta_and_stage(self, regatta_name, stage_number):
        if not regatta_name or not stage_number:
            return
...
if __name__ == "__main__":
    root = tk.Tk()
    race_results_app = RaceResultsApp(root)
    root.mainloop()
#testy
#G